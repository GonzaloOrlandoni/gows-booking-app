...
import { Calendar as CalendarIcon, User, Stethoscope, CheckCircle2, ChevronRight, ChevronLeft, Clock, Sparkles, Smile } from 'lucide-react';
import { services, professionals, generateAvailableTimeSlots } from '../data/mockData';
...
const getIcon = (iconName) => {
  switch (iconName) {
    case 'stethoscope': return <Stethoscope className="w-6 h-6 text-primary-500" />;
    case 'sparkles': return <Sparkles className="w-6 h-6 text-primary-500" />;
    case 'smile': return <Smile className="w-6 h-6 text-primary-500" />;
    default: return <Stethoscope className="w-6 h-6 text-primary-500" />;
  }
};
          ...
          <div className="flex items-center justify-between">
            {steps.map((s, idx) => (
              <div key={s.num} className="flex flex-col items-center relative z-10 flex-1">
                ...
                <span className={`text-xs mt-2 font-medium ${step >= s.num ? 'text-primary-800' : 'text-gray-400'}`}>
                  {s.label}
                </span>
                {idx < steps.length - 1 && (
                  <div className={`absolute top-5 left-1/2 w-full h-1 -z-10 transition-colors duration-300 ${
        ...
        <AnimatePresence mode="wait">
          {isSearching && (
            ...
            >
              ...
              <p className="text-primary-800 font-bold text-lg animate-pulse">Consultando disponibilidad...</p>
              <p className="text-gray-500 text-sm mt-2">Estamos conectando con la agenda de {formData.professional?.name}</p>
            </motion.div>
          )}
          ...
          {step === 1 && (
            <motion.div
              ...
              className="mt-8"
            >
              <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">¿Qué tratamiento necesitas?</h2>
          ...
          {step === 2 && (
            <motion.div
              ...
              className="mt-8"
            >
              <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Elige tu profesional</h2>
                  ...
                  >
                    <div className="h-48 overflow-hidden relative">
                      ...
                      <div className="absolute bottom-4 left-4 text-white">
                        ...
                        <p className="text-sm text-gray-200">{prof.specialty}</p>
                      </div>
                    </div>
          ...
          {step === 3 && (
            <motion.div
              ...
              className="mt-8"
            >
              <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Selecciona Fecha y Hora</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="border border-gray-200 rounded-xl p-5 bg-white">
                  ...
                  <div className="space-y-3">
                    {[0, 1, 2, 3, 4].map(offset => {
                      ...
                      const isSelected = formData.date && isSameDay(formData.date, date);
                      
                      return (
                        <div 
                          key={offset}
                          onClick={() => setFormData({...formData, date, time: null})}
                        ...
                        >
                          <span className="capitalize">{format(date, 'EEEE d, MMMM', { locale: es })}</span>
                ...
                <div className={`transition-opacity duration-300 ${formData.date ? 'opacity-100' : 'opacity-50 pointer-events-none'}`}>
                  <h3 className="font-semibold text-gray-700 mb-4 flex items-center">
                    <Clock className="w-5 h-5 mr-2 text-primary-500" />
                    Horarios {formData.date && 'para el ' + format(formData.date, 'd/MM')}
                  </h3>
          ...
          {step === 4 && (
            <motion.div
              ...
              className="mt-8"
            >
              <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Completa tus datos</h2>
              
              <div className="bg-primary-50 rounded-xl p-5 mb-8 border border-primary-100">
                <h3 className="font-semibold text-primary-900 mb-3">Resumen de tu turno:</h3>
                <ul className="space-y-2 text-sm text-primary-800">
                  <li className="flex items-center"><Stethoscope className="w-4 h-4 mr-2" /> <strong>Servicio:</strong> {formData.service?.name}</li>
                  <li className="flex items-center"><User className="w-4 h-4 mr-2" /> <strong>Profesional:</strong> {formData.professional?.name}</li>
                  <li className="flex items-center"><CalendarIcon className="w-4 h-4 mr-2" /> <strong>Fecha:</strong> {formData.date && format(formData.date, 'EEEE d, MMMM', { locale: es })} a las {formData.time}</li>
                  <li className="flex items-center text-primary-600 font-semibold mt-2 border-t border-primary-200 pt-2">Total a abonar en clínica: {formData.service?.price}</li>
                </ul>
              </div>
          ...
          {step === 5 && (
            ...
            >
              ...
              <h2 className="text-3xl font-bold text-gray-800 mb-4">¡Turno Confirmado!</h2>
              <p className="text-gray-600 max-w-md mx-auto mb-8 text-lg">
                Gracias, <strong>{formData.patientName}</strong>. Te hemos enviado un mensaje de WhatsApp al <strong>{formData.patientPhone}</strong> con los detalles de tu turno para el {formData.date && format(formData.date, 'd/MM/yyyy')} a las {formData.time}.
              </p>
              
              <button 
                onClick={() => {
                  setStep(1);
                  setFormData({ service: null, professional: null, date: null, time: null, patientName: '', patientPhone: '' });
                }}
